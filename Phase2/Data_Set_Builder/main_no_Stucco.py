...
def phase_from_weights(w):
    ...
    :param w: 5x1 numpy array containing weight of each constituent:
                ...
                w[3] = weight of inert sample
    :return: p, 4x1 numpy array containing phase contents resulted from the combinations of above weights
    """

    p = np.zeros(4)
    w_sum = np.sum(w)
    p[0] = 100 * (w[0] * dh_sample_purity) / w_sum
    p[1] = 100 * (w[1] * hh_sample_purity) / w_sum
    p[2] = 100 * (w[2] * a3_sample_purity) / w_sum
    p[3] = 100 - p[0] - p[1] - p[2]
...
costs = np.array([0.5360, 0.0599, 0.0599, 0])
